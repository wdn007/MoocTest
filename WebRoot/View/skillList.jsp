...
<head>
	...
	<body>
		...
		<div class="container listPage-container" style="margin-top: 0;">
			...
			<div id="courseBox">
				<c:forEach items="${ skillArticles }" var="skillList">
				<div class="course-list skill-list">
					<h1><span>【${ skillList.classification }】</span>${ skillList.title }</h1>
					<h2><span>${ skillList.userId }&nbsp;&nbsp;</span>发表于:<span>${ skillList.createDate }</span></h2>
					<p class="content-p">
						${ skillList.content }
					</p>
					<p>关键词：
						<c:forEach var="keyword" items="${ skillList.keyword }">
							<a>${ keyword }</a>
						</c:forEach>
					</p>
